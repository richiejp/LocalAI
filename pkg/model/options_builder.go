...
func BuildModelOptions(c config.ModelConfig, so *config.ApplicationConfig, opts ...Option) []Option {
	name := c.Name
	if name == "" {
		name = c.Model
	}
	...
	defOpts := []Option{
		WithBackendString(c.Backend),
		WithModel(c.Model),
		WithContext(so.Context),
		WithModelID(name),
	}
	...
	defOpts = append(defOpts, WithLoadGRPCLoadModelOpts(grpcOpts))

	if so.ParallelBackendRequests {
		defOpts = append(defOpts, EnableParallelRequests)
	}

	if c.GRPC.Attempts != 0 {
		defOpts = append(defOpts, WithGRPCAttempts(c.GRPC.Attempts))
	}

	if c.GRPC.AttemptsSleepTime != 0 {
		defOpts = append(defOpts, WithGRPCAttemptsDelay(c.GRPC.AttemptsSleepTime))
	}

	for k, v := range so.ExternalGRPCBackends {
		defOpts = append(defOpts, WithExternalBackend(k, v))
	}
