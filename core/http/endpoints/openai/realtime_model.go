...
func (m *wrappedModel) Predict(ctx context.Context, messages schema.Messages, images, videos, audios []string, tokenCallback func(string, backend.TokenUsage) bool, tools []types.ToolUnion, toolChoice *types.ToolChoiceUnion, logprobs *int, topLogprobs *int, logitBias map[string]float64) (func() (backend.LLMResponse, error), error) {
	...
	// Cloud dispatch short-circuit: when the router landed on a proxy-*
	// backend, the rest of the local path (templating, grammar, gRPC)
	// is irrelevant — the cloud API does its own templating against the
	// OpenAI request shape. Stream tokens straight from cloudproxy.
	if turnCfg.IsCloudProxy() {
		return m.predictViaCloud(ctx, turnCfg, &input, tokenCallback)
	}
...
func (m *wrappedModel) routeTurn(ctx context.Context, req *schema.OpenAIRequest) (*config.ModelConfig, error) {
	...
	if m.routerStore != nil {
		_ = m.routerStore.Record(context.Background(), router.DecisionRecord{
			ID:              newRealtimeDecisionID(),
			CorrelationID:   m.routerSessionID,
			UserID:          m.routerUserID,
...
// predictViaCloud dispatches the turn to a proxy-* cloud LLM and adapts
// the streaming token callback shape to backend.LLMResponse so the
// realtime caller sees the same Predict() contract as the local path.
// Cloud errors are wrapped in a cloudproxy.UpstreamError so the
// realtime handler can relay the upstream's original status/body to its
// client unchanged — matching the fidelity we already give HTTP traffic.
func (m *wrappedModel) predictViaCloud(ctx context.Context, cfg *config.ModelConfig, req *schema.OpenAIRequest, tokenCallback func(string, backend.TokenUsage) bool) (func() (backend.LLMResponse, error), error) {
	return func() (backend.LLMResponse, error) {
		return cloudproxy.StreamTokens(ctx, cfg, req, func(token string, usage backend.TokenUsage) bool {
			if tokenCallback == nil {
				return true
			}
			return tokenCallback(token, usage)
		})
	}, nil
}
...
// when no scorer factory is wired (e.g. tests, stripped builds) — that
// path leaves wrappedModel.Predict on the historical "no routing" path
...
func buildRealtimeRoutingContext(a *application.Application, sessionID string) *RealtimeRoutingContext {
	...
	scorer := a.ScorerFactory()
	reranker := a.RerankerFactory()
	if scorer == nil && reranker == nil {
		return nil
	}
	deps := &middleware.ClassifierDeps{
		Scorer:      scorer,
		Embedder:    a.EmbedderFactory(),
		VectorStore: a.VectorStoreFactory(),
		Reranker:    reranker,
