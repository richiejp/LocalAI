...
var usecaseFilters = map[string]config.ModelConfigUsecase{
	"llm":       config.FLAG_CHAT,
	"image":     config.FLAG_IMAGE,
	"vision":    config.FLAG_VISION,
	"tts":       config.FLAG_TTS,
	"stt":       config.FLAG_TRANSCRIPT,
	"embedding": config.FLAG_EMBEDDINGS,
	"reranker":  config.FLAG_RERANK,
}

// flagToUIKey is the reverse of usecaseFilters: ModelConfigUsecase → UI key.
// Computed once at init to avoid rebuilding per request.
var flagToUIKey = func() map[config.ModelConfigUsecase]string {
	m := make(map[config.ModelConfigUsecase]string, len(usecaseFilters))
	for uiKey, flag := range usecaseFilters {
		m[flag] = uiKey
	}
	return m
}()
...
func RegisterUIAPIRoutes(app *echo.Echo, cl *config.ModelConfigLoader, ml *model.ModelLoader, appConfig *config.ApplicationConfig, galleryService *galleryop.GalleryService, opcache *galleryop.OpCache, applicationInstance *application.Application, adminMiddleware echo.MiddlewareFunc) {
	...
	// Returns a mapping of backend names to the UI filter keys they support.
	...
	app.GET("/api/backends/usecases", func(c echo.Context) error {
		...
		for name, cap := range config.BackendCapabilities {
			...
			for _, uc := range cap.PossibleUsecases {
				if info, ok := config.UsecaseInfoMap[uc]; ok {
					if uiKey, ok := flagToUIKey[info.Flag]; ok {
						if !slices.Contains(keys, uiKey) {
