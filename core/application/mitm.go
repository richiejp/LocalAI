...
func startMITMLocked(app *Application, options *config.ApplicationConfig) error {
	...
	// A backend without "proxy-" prefix and no explicit pii.enabled
	// resolves to false → host is intercepted but the regex pass is
	// skipped (audit events still record).
