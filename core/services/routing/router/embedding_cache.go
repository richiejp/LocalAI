...
// Embedder produces a fixed-dimension vector from a prompt. The router
// uses it to look up semantically-similar past decisions in the L2
// cache. Implementations honour ctx cancellation so a slow embedder
// doesn't pin the request goroutine.
type Embedder interface {
	Embed(ctx context.Context, text string) ([]float32, error)
}

// VectorStore is the KNN backend the embedding cache writes to. Search
// returns the top-1 match (similarity in [-1, 1] cosine space) and the
// serialized Decision payload, or ok=false on a clean miss. Insert
// stores a new (vector, decision) pair; size and eviction are the
...
func NewEmbeddingCacheClassifier(inner Classifier, embedder Embedder, store VectorStore, similarityThreshold, confidenceThreshold float64) *EmbeddingCacheClassifier {
	if inner == nil {
