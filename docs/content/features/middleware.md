...
```yaml
# Cloud-bound model — defaults to enabled because backend starts with
# proxy-. Tighten api_key_prefix from the global default and downgrade
# email to route_local so emails route to a local model rather than
# leaving the network.
name: claude-strict
...
`smart-router → claude-strict → proxy-anthropic` chain is fine
(`claude-strict` is a regular proxy model). A
